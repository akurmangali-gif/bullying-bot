...
async def cmd_terms(message: Message):
    await message.answer(
        ...
        "— Базовый пакет (Документы 1–3) — 3 900 тг\n"
        "— Полный пакет (Документы 1–4 + напоминания о сроках) — 5 900 тг\n"
        "— Консультация юриста ADDASTRA — 25 000 тг\n\n"
        "<b>3. Что не входит</b>\n"
...
async def cmd_history(message: Message):
    ...
    await message.answer(
        "\n\n".join(lines) + "\n\n<i>Для нового обращения нажмите кнопку ниже — "
        "анкета займёт около 3 минут.</i>",

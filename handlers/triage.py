...
DISCLAIMER = (
    "⚠️ <b>Важно:</b> Этот бот предоставляет процедурный маршрут и шаблоны "
    "документов в соответствии с Правилами № 506 МП РК. Бот <b>не оказывает "
    "юридических услуг</b> и не гарантирует конкретный результат. "
    "Если ситуация сложная — обратитесь к юристу."
)
...
def level_advice(level: str, systematic: bool) -> str:
    ...
    if level == "AMBER":
        base = ("🟡 <b>Рекомендация:</b> Параллельно подайте документы в школу "
                ...
                "Если <b>до 14 лет</b> — основной трек через школу и акимат.")
    else:
        base = ("🟢 <b>Рекомендация:</b> Действуем через школьную процедуру по Правилам № 506: "
                "заявление директору → Совет профилактики → при необходимости акимат.")
...
async def show_red_alert(message: Message, state: FSMContext):
    await message.answer(
        ...
        "документов для школы и акимата.\n\n"

...
async def generate_and_send(message: Message, state: FSMContext):
    ...
    level_tip = {
        "GREEN": (
            ...
            "3. Если школа не реагирует — обращайтесь в отдел/управление образования управление образованияа"
        ),
        "AMBER": (
            "🟡 <b>Следующие шаги:</b>\n"
    ...
    if level == "AMBER":
        ...
        if "14–15" in age:
            ...
            age_tip = "уголовная ответственность не наступает — трек через школу и управление образования"
        else:
